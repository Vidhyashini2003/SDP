import { useState, useEffect } from 'react';
import axios from '../../config/axios';
import { toast } from 'react-hot-toast';

const ActivityManagement = () => {
    const [activities, setActivities] = useState([]);
    const [loading, setLoading] = useState(true);
    const [isAdding, setIsAdding] = useState(false);
    const [editingItem, setEditingItem] = useState(null);
    const [imageFile, setImageFile] = useState(null);
    const [formData, setFormData] = useState({
        activity_name: '',
        activity_price_per_hour: '',
        activity_status: 'Available',
        activity_image: ''
    });

    useEffect(() => {
        fetchActivities();
    }, []);

    const fetchActivities = async () => {
        try {
            const res = await axios.get('/api/receptionist/activities');
            setActivities(res.data);
            setLoading(false);
        } catch (error) {
            console.error('Error fetching activities:', error);
            setLoading(false);
        }
    };

    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const data = new FormData();
            data.append('activity_name', formData.activity_name);
            data.append('activity_price_per_hour', formData.activity_price_per_hour);
            data.append('activity_status', formData.activity_status);
            if (imageFile) {
                data.append('image', imageFile);
            } else if (formData.activity_image) {
                data.append('activity_image', formData.activity_image);
            }

            if (editingItem) {
                await axios.put(`/api/receptionist/manage/activities/${editingItem.activity_id}`, data, {
                    headers: { 'Content-Type': 'multipart/form-data' }
                });
                toast.success('Activity updated successfully');
            } else {
                await axios.post('/api/receptionist/manage/activities', data, {
                    headers: { 'Content-Type': 'multipart/form-data' }
                });
                toast.success('Activity added successfully');
            }
            resetForm();
            fetchActivities();
        } catch (error) {
            toast.error(editingItem ? 'Failed to update activity' : 'Failed to add activity');
        }
    };

    const handleEdit = (activity) => {
        setEditingItem(activity);
        setFormData({
            activity_name: activity.activity_name,
            activity_price_per_hour: activity.activity_price_per_hour,
            activity_status: activity.activity_status,
            activity_image: activity.activity_image || ''
        });
        setIsAdding(true);
        window.scrollTo({ top: 0, behavior: 'smooth' });
    };

    const handleDelete = async (id) => {
        if (!window.confirm('Are you sure you want to delete this activity?')) return;
        try {
            await axios.delete(`/api/receptionist/manage/activities/${id}`);
            toast.success('Activity deleted');
            fetchActivities();
        } catch (error) {
            toast.error(error.response?.data?.error || 'Failed to delete activity');
        }
    };

    const resetForm = () => {
        setFormData({ activity_name: '', activity_price_per_hour: '', activity_status: 'Available', activity_image: '' });
        setImageFile(null);
        setEditingItem(null);
        setIsAdding(false);
    };

    if (loading) return <div className="p-8 text-center text-slate-500">Loading activities...</div>;

    return (
        <div>
            <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-bold text-slate-800">Activity Management</h2>
                <button
                    onClick={() => { if (isAdding) resetForm(); else setIsAdding(true); }}
                    className="px-4 py-2 bg-gold-600 hover:bg-gold-700 text-white rounded-lg font-semibold transition-colors shadow-sm"
                >
                    {isAdding ? 'Cancel' : '+ Add Activity'}
                </button>
            </div>

            {isAdding && (
                <div className="mb-8 bg-white p-6 rounded-xl shadow-lg border border-slate-200">
                    <h3 className="text-lg font-semibold mb-4 text-slate-800">
                        {editingItem ? `Edit ${editingItem.activity_name}` : 'Add New Activity'}
                    </h3>
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            <div className="lg:col-span-2">
                                <label className="block text-sm font-medium text-slate-700 mb-1">Activity Name</label>
                                <input
                                    type="text"
                                    required
                                    value={formData.activity_name}
                                    onChange={e => setFormData({ ...formData, activity_name: e.target.value })}
                                    className="w-full px-3 py-2 border border-slate-300 rounded-lg outline-none focus:ring-2 focus:ring-gold-500"
                                    placeholder="e.g. Snorkeling Adventure"
                                />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-slate-700 mb-1">Price Per Hour (Rs.)</label>
                                <input
                                    type="number"
                                    required
                                    value={formData.activity_price_per_hour}
                                    onChange={e => setFormData({ ...formData, activity_price_per_hour: e.target.value })}
                                    className="w-full px-3 py-2 border border-slate-300 rounded-lg outline-none focus:ring-2 focus:ring-gold-500"
                                    placeholder="0.00"
                                />
                            </div>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label className="block text-sm font-medium text-slate-700 mb-1">Status</label>
                                <select
                                    value={formData.activity_status}
                                    onChange={e => setFormData({ ...formData, activity_status: e.target.value })}
                                    className="w-full px-3 py-2 border border-slate-300 rounded-lg outline-none focus:ring-2 focus:ring-gold-500"
                                >
                                    <option value="Available">Available</option>
                                    <option value="Unavailable">Unavailable</option>
                                </select>
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-slate-700 mb-1">{editingItem ? 'Change Image' : 'Upload Image'}</label>
                                <div className="flex items-center gap-4">
                                    {editingItem && formData.activity_image && !imageFile && (
                                        <img
                                            src={formData.activity_image.startsWith('/') ? `${axios.defaults.baseURL}${formData.activity_image}` : formData.activity_image}
                                            className="w-12 h-12 rounded-lg object-cover border border-slate-200"
                                            alt="Current"
                                        />
                                    )}
                                    {imageFile && (
                                        <img
                                            src={URL.createObjectURL(imageFile)}
                                            className="w-12 h-12 rounded-lg object-cover border-2 border-gold-500"
                                            alt="Preview"
                                        />
                                    )}
                                    <input
                                        type="file"
                                        accept="image/*"
                                        onChange={e => setImageFile(e.target.files[0])}
                                        className="flex-1 px-3 py-1.5 border border-slate-300 rounded-lg outline-none file:mr-4 file:py-1 file:px-2 file:rounded-md file:border-0 file:text-xs file:font-semibold file:bg-gold-50 file:text-gold-700"
                                    />
                                </div>
                            </div>
                        </div>
                        <div className="flex gap-2">
                            <button type="submit" className="px-6 py-2 bg-slate-900 hover:bg-slate-800 text-white rounded-lg font-bold shadow-md">
                                {editingItem ? 'Update Activity' : 'Save Activity'}
                            </button>
                            {editingItem && (
                                <button type="button" onClick={resetForm} className="px-6 py-2 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-lg font-bold">
                                    Cancel
                                </button>
                            )}
                        </div>
                    </form>
                </div>
            )}

            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                <table className="w-full text-left">
                    <thead className="bg-slate-50 border-b border-slate-200">
                        <tr>
                            <th className="px-6 py-4 font-bold text-slate-600 uppercase text-[10px] tracking-widest">Activity</th>
                            <th className="px-6 py-4 font-bold text-slate-600 uppercase text-[10px] tracking-widest">Price/Hour</th>
                            <th className="px-6 py-4 font-bold text-slate-600 uppercase text-[10px] tracking-widest">Status</th>
                            <th className="px-6 py-4 font-bold text-slate-600 uppercase text-[10px] tracking-widest text-right">Actions</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                        {activities.map(activity => (
                            <tr key={activity.activity_id} className="hover:bg-slate-50 transition-colors">
                                <td className="px-6 py-4">
                                    <div className="flex items-center gap-3">
                                        {activity.activity_image ? (
                                            <img
                                                src={activity.activity_image.startsWith('/') ? `${axios.defaults.baseURL}${activity.activity_image}` : activity.activity_image}
                                                alt={activity.activity_name}
                                                className="w-12 h-12 rounded-lg object-cover border border-slate-200"
                                            />
                                        ) : (
                                            <div className="w-12 h-12 rounded-lg bg-slate-100 flex items-center justify-center text-slate-400 text-[10px] font-bold">NO IMG</div>
                                        )}
                                        <span className="font-bold text-slate-900">{activity.activity_name}</span>
                                    </div>
                                </td>
                                <td className="px-6 py-4 text-slate-600">Rs. {Number(activity.activity_price_per_hour).toLocaleString()}</td>
                                <td className="px-6 py-4">
                                    <span className={`px-2 py-1 rounded-md text-[10px] font-black uppercase tracking-wider ${
                                        activity.activity_status === 'Available'
                                            ? 'bg-green-100 text-green-700 border border-green-200'
                                            : 'bg-slate-100 text-slate-500 border border-slate-200'
                                    }`}>
                                        {activity.activity_status}
                                    </span>
                                </td>
                                <td className="px-6 py-4 text-right">
                                    <div className="flex justify-end gap-2">
                                        <button onClick={() => handleEdit(activity)} className="text-[10px] font-black uppercase tracking-widest px-3 py-1.5 rounded-md border border-blue-200 text-blue-600 hover:bg-blue-50">
                                            Edit
                                        </button>
                                        <button onClick={() => handleDelete(activity.activity_id)} className="text-[10px] font-black uppercase tracking-widest px-3 py-1.5 rounded-md border border-red-200 text-red-600 hover:bg-red-50">
                                            Delete
                                        </button>
                                    </div>
                                </td>
                            </tr>
                        ))}
                    </tbody>
                </table>
                {activities.length === 0 && (
                    <div className="p-20 text-center text-slate-400 italic">No activities found.</div>
                )}
            </div>
        </div>
    );
};

export default ActivityManagement;
